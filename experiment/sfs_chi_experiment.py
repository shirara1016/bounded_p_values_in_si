...
from source.sfs_chi import SfsSelectiveInferenceChi
...
class SfsSIChiExperiment(PararellExperiment):
    def __init__(
        self,
        num_results: int,
        num_worker: int,
    ...
    def iter_experiment(self, args) -> tuple:
        ...
        X, y, test_indexes = args
        si = SfsSelectiveInferenceChi(X, y, 1, 5, test_indexes)
        si.construct_P()
        ...
        try:
            ...
            res.append(1 - chi.cdf(si.si_calculator.stat, si.si_calculator.degree))
            res = tuple(res)
    ...
    def run_experiment(self):
        ...
        for _ in range(self.num_iter):
            ...
            test_indexes = self.rng.choice(k, 1 + self.rng.choice(k), replace=False)
            X_list.append(X)
            y_list.append(y)
            test_indexes_list.append(test_indexes)

        dataset = zip(X_list, y_list, test_indexes_list)
        ...
        result_dict["naive_p_values"] = [item[i] for item in results]
...
if __name__ == "__main__":
    ...
    file_name = f"sfs_chi_seed{args.seed}_n{args.num_samples}_delta{args.signal}.pkl"
    print(file_name)
    print()
